% decodeMeanFieldPoisson_ECOC.m
clear; close all;

files     = {'MF_responses_OPM.mat','MF_responses_SaltPepper.mat'};
titles    = {'OPM','Salt-and-Pepper'};
groupSize = 100;

for i = 1:2
    fprintf('\n=== %s Mean-Field via Poisson ===\n', titles{i});
...
respMeanCounts(respMeanCounts < 0) = 0;
    ...
    % === [after computing `acc` on true labels] ===
    ...
    % Plot & save
    fig = figure('Name',['MF — ' titles{i}], 'Color','w');
    imagesc(cm); axis square; colormap(parula); colorbar;
    title(sprintf('%s MF — Acc = %.1f%%', titles{i}, acc*100));
    xlabel('Predicted'); ylabel('True');
    xticks(1:numel(stimV)); xticklabels(stimV);
    yticks(1:numel(stimV)); yticklabels(stimV);
    outFig = sprintf('MF_results_%s.png', titles{i});
    saveas(fig, outFig);
    close(fig);
    
    % Save results
    outMat = sprintf('MF_results_%s.mat', titles{i});
